...
void GrassDoWind_float(float deformAmount, float windSpeed, float windScale, float windInfluence, float4 waveTint, float4 color, float4 vertex, out float4 vertOut, out float4 colorOut)
{
	...
	vertex -= mul(unity_WorldToObject, float4(move.x, 0, move.y, 0));
	vertOut = vertex;
	colorOut = color;
}